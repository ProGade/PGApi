...
*/
class classPG_TwitterShare extends classPG_ClassBasics
{
	// Declarations...
	private $sUrlToShare = '';
	private $sTextToShare = '';
	private $sLanguage = 'en';
	private $sShareOnUrl = 'http://twitter.com/share';
	private $sButtonImage = 'twitter_share.png';
	...
	public function __construct()
	{
		...
		$this->setText(array(
			'ShareLinkText' => 'share on twitter'
		));
	}
	...
	*/
	...
	@param sLanguage
	*/
	public function build($_sUrlToShare = NULL, $_sTextToShare = NULL, $_sLanguage = NULL)
	{
		...
		$_sHTML = '';
		$_sHTML .= '<a href="javascript:;" onclick="oPGBrowser.popup(\''.$this->sShareOnUrl;
		$_sHTML .= '?url='.htmlentities(urlencode($_sUrlToShare));
		$_sHTML .= '&amp;text='.urlencode(utf8_encode($_sTextToShare));
		$_sHTML .= '&amp;lang='.$_sLanguage;
		$_sHTML .= '\', '.$this->iPopupWindowSizeX.', '.$this->iPopupWindowSizeY.', \'twitter_share\', ';
...
/* @end class */
$oPGTwitterShare = new classPG_TwitterShare();
if (isset($oPGApi)) {$oPGApi->register(array('sName' => 'oPGTwitterShare', 'xValue' => $oPGTwitterShare));}
?>